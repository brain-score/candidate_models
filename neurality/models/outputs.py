...
def _arrange(layer_name_output, pca_components):
    ...
    return layer_name, arrange_layer_output(layer_output, pca_components=pca_components)


def arrange_layer_outputs(outputs, pca_components, multiprocessing=False):
    if multiprocessing:
        with Pool() as pool:
            arranged_outputs = pool.map(functools.partial(_arrange, pca_components=pca_components), outputs.items())
    else:
        arranged_outputs = map(functools.partial(_arrange, pca_components=pca_components), outputs.items())
    return dict(arranged_outputs)
